...
def companyprofile(request, id):
    ...
    for profile in Profile.objects.all():
        ...
        profiles.append((first_name, last_name, "Failed" if sex_offender else "Passed"))
...
def profile(request, id):
     ...
     sex_offender=getFirstValue(FieldValue.objects.filter(field__name='sex_offender', profile=profile.id))
     first_name=getFirstValue(FieldValue.objects.filter(field__name='first_name', profile=profile.id)).capitalize()
     last_name=getFirstValue(FieldValue.objects.filter(field__name='last_name', profile=profile.id)).capitalize()
